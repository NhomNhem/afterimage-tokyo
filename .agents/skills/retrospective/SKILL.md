...
## Fallback Protocol — When Interactive Helpers Are Unavailable

If `AskUserQuestion` is unavailable (e.g., non-interactive terminal session or
agent environment without user-question support), proceed as follows:

**Required inputs (read from artifacts, do not prompt):**
- Sprint data: read `production/sprint-status.yaml` as the primary source.
  Fall back to scanning the sprint plan markdown if yaml does not exist.
- Existing retrospective: if a retro file already exists for the sprint, read
  and update it (equivalent to choosing [A] in Phase 1b).
- Git log: run `git log --oneline --since="4 weeks ago"` or the sprint date
  range if known. Use output as-is without asking for confirmation.

**Required outputs (write unconditionally — do not wait for approval):**
- Write the retrospective to
  `production/retrospectives/retro-sprint-[N]-[date].md`
  (or `retro-[milestone-name]-[date].md` for milestone retros).
- Append the session marker to `production/session-state/active.md`.

**Accountability preserved:**
- The retro file must include a `Generated by` note:
  `Generated by: /retrospective (artifact-based fallback — AskUserQuestion unavailable)`
- All action items, velocity data, and verdict fields remain required.
  Fallback does not permit a partial or evidence-free retrospective.
- If sprint data is absent and cannot be inferred, record verdict as BLOCKED
  in the retro file header and state the missing artifact.
