...
- Scope: default to `sprint` — read the most recent sprint file in
  `production/sprints/` and `production/sprint-status.yaml`.
- Story files: load all paths listed in the sprint file or yaml.
- GDDs/ADRs: load referenced files as normal.

**Required outputs (write unconditionally — do not wait for approval):**
- Write the QA plan to `production/qa/qa-plan-[sprint-slug]-[date].md`.
- Append the session marker to `production/session-state/active.md`.

**Accountability preserved:**
- The plan file must include a `Generated by` header noting fallback mode:
  `Generated by: /qa-plan (artifact-based fallback — AskUserQuestion unavailable)`
- Evidence files, verdicts, and session markers remain required for story
  close-out. Fallback does not permit evidence-free sign-off.
- If story file paths are missing, record them as MISSING in the plan rather
